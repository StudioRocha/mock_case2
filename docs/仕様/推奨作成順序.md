...
-   [x] `Admin/StaffController` の作成
-   [x] `routes/web.php` に `/admin/staff/list` ルート追加
-   [x] `resources/views/admin/staff/list.blade.php` の作成
-   [x] スタッフ一覧表示機能の実装

### 13. スタッフ別勤怠一覧画面（PG11）

-   [x] `Admin/AttendanceController@staff` メソッドの追加
-   [x] `routes/web.php` に `/admin/attendance/staff/{id}` ルート追加
-   [x] `resources/views/admin/attendance/staff.blade.php` の作成
-   [x] スタッフ別勤怠一覧表示機能の実装
-   [x] 月次表示機能の実装

-   [x] 応用要件　　 FN045 CSV 出力機能 「CSV 出力」を押下すると，当該ユーザーが選択した月で行った勤怠一覧情報が CSV でダウンロードできること

### 14. 申請一覧画面（管理者）（PG12）

-   [x] `Admin/StampCorrectionRequestController` の作成（または既存コントローラーの拡張）
-   [x] `resources/views/admin/stamp_correction_request/list.blade.php` の作成
-   [x] 管理者向け申請一覧表示機能の実装

### 15. 修正申請承認画面（PG13）

-   [x] `Admin/StampCorrectionRequestController@show` メソッドの追加（修正申請詳細表示）
-   [x] `Admin/StampCorrectionRequestController@processApprove` メソッドの追加（承認処理）
-   [x] `routes/web.php` に `/stamp_correction_request/detail/{id}` と `/stamp_correction_request/approve/{id}` ルート追加
-   [x] `resources/views/admin/stamp_correction_request/detail.blade.php` の作成
-   [x] 承認処理の実装（FN051: 承認機能）
-   [x] 勤怠情報更新処理の実装（修正申請の内容を実際の勤怠データに反映）
