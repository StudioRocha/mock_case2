...
-   [ ] `Admin/AttendanceController@show` メソッドの実装（FN037: 詳細情報取得機能）
-   [ ] `Admin/AttendanceController@update` メソッドの実装（FN040: 修正機能）
-   [ ] `routes/web.php` に `/admin/attendance/{id}` ルート追加（GET）
-   [ ] `routes/web.php` に `/admin/attendance/{id}` ルート追加（POST: 修正処理）
-   [ ] `resources/views/admin/attendance/show.blade.php` の作成
    -   [ ] 一般ユーザー用の`attendance/detail.blade.php`をベースに作成
    -   [ ] ヘッダーメニューは共通レイアウトで自動的に管理者用に切り替わる
    -   [ ] フォームの action を管理者用の修正ルートに設定
-   [ ] FN037: 詳細情報取得機能の実装
    -   [ ] 動線上で選択した情報と一致することを確認
    -   [ ] 実際の勤怠内容が正しく反映されることを確認
-   [ ] FN038: 項目編集機能の実装
    -   [ ] 「出勤・退勤」「休憩」「備考」の 4 つの項目を編集可能にする
    -   [ ] 承認待ちの申請詳細がある場合は修正不可にし、「承認待ちのため修正はできません。」とメッセージ表示
-   [ ] FN039: バリデーション機能の実装（FormRequest 使用）
    -   [ ] 出勤時間が退勤時間より後の場合のエラーメッセージ
    -   [ ] 退勤時間が出勤時間より前の場合のエラーメッセージ
    -   [ ] 休憩開始時間が出勤時間より前の場合のエラーメッセージ
    -   [ ] 休憩開始時間が退勤時間より後の場合のエラーメッセージ
    -   [ ] 休憩終了時間が退勤時間より後の場合のエラーメッセージ
    -   [ ] 備考欄が未入力の場合のエラーメッセージ
-   [ ] FN040: 修正機能の実装
    -   [ ] 「修正」ボタン押下で管理者として直接修正を実行
    -   [ ] 修正された内容が一般ユーザーの勤怠情報にも反映されることを確認
