...
-   **フロントエンド**: `disabled`属性で入力フィールドを無効化
-   **バリデーション**: FormRequest で承認待ちをチェックし、エラーを返す
-   **コントローラー**: `update`メソッドで承認待ちをチェックし、リダイレクト
-   **バリデーション失敗時**: ページエラー（500）ではなく、バリデーションエラーとして処理され、前のページにリダイレクトしてエラーメッセージを表示
...
1. **定数の使用**: コード内では数値（`0`, `1`,）を直接書かず、必ず定数（`STATUS_PENDING`, `STATUS_APPROVED`, ）を使用すること
