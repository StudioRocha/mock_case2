...
-   `user_id`と`date`の組み合わせで UNIQUE 制約（1 ユーザー 1 日 1 レコード）
-   `user_id`カラム単独、`date`カラム単独ではユニークキーではなく、`user_id`と`date`の組み合わせでユニーク制約が設定されています
...
## stamp_correction_requests テーブル（勤怠修正申請）
