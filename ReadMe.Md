...
**補足**: この ReadMe は基本的には WindowsOS での環境構築をメインに記したものです。

### Laravel 環境構築

**注意**: Windows WSL 環境において PHP コマンドを実行する場合は、以下のコマンドでコンテナに入ることを推奨します：

```bash
docker-compose exec -u www-data php bash
```

これにより、`www-data`ユーザーでコンテナに入ることができ、ファイル所有権の問題を防ぐことができます。

#### 1. PHP コンテナ上 - コンポーサーのインストール

```bash
composer install
```

#### 2. .env.example から .env を作成し、以下の環境変数を設定または追加。
...
#### 3. PHP コンテナ上 - アプリケーションキーの作成

```bash
php artisan key:generate
```

#### 4. PHP コンテナ上 - マイグレーションの実行

```bash
php artisan migrate
```

#### 5. PHP コンテナ上 - 全キャッシュクリア（一括）
...
#### 7. PHP コンテナ上 - シーディングの実行（ダミーデータの一括生成）
