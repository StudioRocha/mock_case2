...
8. ストレージリンク作成(今回のアプリではこのコマンドは必要ありません)<BR>
   <BR> PHP コンテナ上
...
2. **メール認証方法**:
    ...
    **方法 A: メール認証誘導画面から MailHog を開く**
    ...
    - メール認証誘導画面では、認証完了を自動検知するポーリング機能が動作しています
    - 認証が完了すると、自動的に勤怠登録画面にリダイレクトされます
    ...
    **方法 B: MailHog 受信箱から直接認証**
    ...
    - メール認証が完了し、認証完了画面（`/email/verify/complete`）にリダイレクトされます
    - 認証完了画面から「勤怠登録画面へ」ボタンをクリックして勤怠登録画面に遷移します
    - または、メール認証誘導画面が開いている場合は、ポーリング機能により自動的に勤怠登録画面にリダイレクトされます
...
-   メール認証誘導画面では、認証完了を検知するためにポーリング機能が動作しています（約 2 秒間隔で認証状態をチェック）
