...
1 つの`attendance`レコード（例: `id = 1`）に対して、`breaks`テーブルに**3 つのレコード**が作成されます：
...
-   **`status`カラムが必要な理由**: 修正申請の承認ワークフローを管理するため
    ...
    -   **'approved'（承認済み）**: 管理者が承認した状態
        ...
        -   承認時に、修正申請の内容が実際の勤怠データ（`attendances`, `breaks`）に反映される（FN051）
    -   **なぜ必要か**:
...
-   **データフロー**:
    ...
    3.  管理者が承認すると、`break_corrections`の内容が`breaks`テーブルに反映される（FN051）
-   **バリデーション要件**（FN029）:
...
-   各レコードが独立した休憩修正情報を保持
-   管理者が承認すると、これらの 3 つのレコードが`breaks`テーブルに反映される
...
-   承認時の反映: `break_corrections`の全レコードを`breaks`テーブルに一括反映
...
3. **CSV 出力**: `attendances`と`breaks`を JOIN して出力可能（FN045）
